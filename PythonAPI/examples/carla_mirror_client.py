...
class CarlaMirrorClient:
    def __init__(self, parent, debug=False, host='127.0.0.1', port=2000, autopilot=False, res='1280x720', filter='vehicle.*', rolename='hero', gamma=2.2):
        self.parent = parent
        self.debug = debug
        self.host = host
        self.port = port
    ...
    class World(object):
        ...
        def restart(self):
            ...
            while self.player is None:
                ...
                self.player = self.world.try_spawn_actor(blueprint, spawn_point)
            # Set up the sensors.
            self.collision_sensor = self._parent.CollisionSensor(self.player, self.hud)
            self.lane_invasion_sensor = self._parent.LaneInvasionSensor(self.player, self.hud)
            self.gnss_sensor = self._parent.GnssSensor(self.player)
            self.imu_sensor = self._parent.IMUSensor(self.player)
            self.camera_manager = self._parent.CameraManager(self.player, self.hud, self._gamma)
    ...
    class KeyboardControl(object):
        ...
        def parse_events(self, client, world):
            ...
            if not self._autopilot_enabled:
                if isinstance(self._control, carla.VehicleControl):
                    self._parse_vehicle_keys(pygame.key.get_pressed(), self._parent.clock.get_time())
                    ...
                    self._parse_walker_keys(pygame.key.get_pressed(), self.clock.get_time(), world)
    ...
    class HUD(object):
        def __init__(self, parent, width, height):
            ...
            self._notifications = self._parent.FadingText(parent, font, (width, 40), (0, height - 40))
        ...
        def tick(self, world):
            ...
            compass = self._parent.world_object.imu_sensor.compass
            heading = 'N' if compass > 270.5 or compass < 89.5 else ''
            heading += 'S' if 90.5 < compass < 269.5 else ''
            heading += 'E' if 0.5 < compass < 179.5 else ''
            heading += 'W' if 180.5 < compass < 359.5 else ''
            colhist = self._parent.world_object.collision_sensor.get_collision_history()
            ...
            self._info_text = [
                ...
                '',
                'Vehicle: % 20s' % get_actor_display_name(self._parent.world_object.player, truncate=20),
                'Map:     % 20s' % self._parent.world_object.map.name.split('/')[-1],
                'Simulation time: % 12s' % datetime.timedelta(seconds=int(self.simulation_time)),
                '',
                'Speed:   % 15.0f km/h' % (3.6 * math.sqrt(v.x**2 + v.y**2 + v.z**2)),
                u'Compass:% 17.0f\N{DEGREE SIGN} % 2s' % (compass, heading),
                'Accelero: (%5.1f,%5.1f,%5.1f)' % (self._parent.world_object.imu_sensor.accelerometer),
                'Gyroscop: (%5.1f,%5.1f,%5.1f)' % (self._parent.world_object.imu_sensor.gyroscope),
                'Location:% 20s' % ('(% 5.1f, % 5.1f)' % (t.location.x, t.location.y)),
                'GNSS:% 24s' % ('(% 2.6f, % 3.6f)' % (self._parent.world_object.gnss_sensor.lat, self._parent.world_object.gnss_sensor.lon)),
            ...
            if len(vehicles) > 1:
                ...
                vehicles = [(distance(x.get_location()), x) for x in vehicles if x.id != self._parent.world_object.player.id]
    ...
    class CollisionSensor(object):
        def __init__(self, parent_actor, hud):
            ...
            self.sensor.listen(lambda event: self.CollisionSensor._on_collision(weak_self, event))
    ...
    class LaneInvasionSensor(object):
        def __init__(self, parent_actor, hud):
            ...
            # reference.
            weak_self = weakref.ref(self)
            self.sensor.listen(lambda event: self.LaneInvasionSensor._on_invasion(weak_self, event))
    ...
    class GnssSensor(object):
        def __init__(self, parent_actor):
            ...
            # reference.
            weak_self = weakref.ref(self)
            self.sensor.listen(lambda event: self._on_gnss_event(weak_self, event))
    ...
    class IMUSensor(object):
        def __init__(self, parent_actor):
            ...
            weak_self = weakref.ref(self)
            self.sensor.listen(
                lambda sensor_data: self._IMU_callback(weak_self, sensor_data))
    ...
    class RadarSensor(object):
        def __init__(self, parent_actor):
            ...
            weak_self = weakref.ref(self)
            self.sensor.listen(
                lambda radar_data: self.RadarSensor._Radar_callback(weak_self, radar_data))
    ...
    class CameraManager(object):
        def __init__(self, parent_actor, hud, gamma_correction):
            ...
            self._camera_transforms = [
                ...
                (carla.Transform(carla.Location(x=-5.5, z=2.5), carla.Rotation(pitch=0.0, yaw=180.0)), Attachment.Rigid)]
            self.transform_index = 1
            self.rear_view_index = len(self._camera_transforms) - 1
            self.rear_view_array = None
            self.sensors = [
        ...
        def get_rear_view_as_numpy(self):
            rear_transform = self._camera_transforms[self.rear_view_index]

            # Check if rear camera sensor exists
            if hasattr(self, 'rear_camera_sensor') and self.rear_camera_sensor is not None:
                # Don't respawn the sensor if it already exists
                return getattr(self, 'rear_view_array', None)

            # Create a new sensor for the rear view if it doesn't exist
            bp = self.sensors[0][-1]  # Using the first camera blueprint (RGB)
            self.rear_camera_sensor = self._parent.get_world().spawn_actor(
                bp,
                rear_transform[0],
                attach_to=self._parent,
            ...
            # Weak reference to avoid circular reference issues
            weak_self = weakref.ref(self)

            # Capture a single image from the rear camera
            def parse_rear_view_image(weak_self, image):
                self = weak_self()
                if not self:
                    return
                ...
                self.rear_view_array = array

            # Attach listener for the rear camera sensor
            self.rear_camera_sensor.listen(lambda image: parse_rear_view_image(weak_self, image))
